...
export default function Button ({text, link, onClick, classname} ) {
    ...
    return (
        ...
        onClick={mobileAnimation ? animateClick : onClick}>
            <button className={` py-[0.5] px-3 cursor-pointer md:px-6 md:py-[0.5] lg:px-5 relative flex items-center justify-center overflow-hidden ${classname}`}>
                ...
                <span className={`relative z-10 transition-colors text-xs md:text-sm  `} style={hover ? {color:bgColor} : {color:txtColor}}>
                    {text}
                </span>
            </button>
        </a>
